...
export function startAuditLoop(cfg: AuditLoopConfig): void {
  ...
  async function runOnce(): Promise<void> {
    ...
    const reportHash = keccak256(stringToHex(JSON.stringify(json))) as Hex;
  ...
  console.log(
    `[audit] scheduler every ${cfg.intervalMs}ms → ${cfg.auditServerUrl} → provider ${cfg.providerId} on ${cfg.marketplaceAddress}`,
  );
